...
def count_images(actor_folder: Path) -> int:
    return sum(
        1 for file_path in actor_folder.rglob("*")
...
def read_metadata_counts():
    ...
    with METADATA_CSV.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        for row in reader:
            ...
            if (
                "NEEDED" in source.upper()
                or "NEEDED" in license_name.upper()
                or "NEEDED" in author.upper()
            ):
                placeholders.append({
                    "actor": actor,
...
def main():
    ...
    for folder in actor_folders:
        ...
        metadata_count = metadata_counts.get(actor_display, 0)
    ...
    if placeholders:
        ...
        for item in placeholders:
            print(f"- {item['actor']} | {item['image_file']}")
    else:
    ...
    print("The dataset folders, metadata tracking, progress tracker, and demo evidence are in place.")
    print("Next work: replace placeholder metadata, collect more verified images, annotate, split, train, and evaluate.")
    print("=" * 70)
    ...
    with REPORT_MD.open("w", encoding="utf-8") as report:
        ...
        for row in demo_rows:
            report.write(f"| {row['folder']} | {row['image_count']} | {row['metadata_count']} |\n")
        ...
        if placeholders:
            for item in placeholders:
                report.write(f"- {item['actor']} — {item['image_file']}\n")
        else:
        ...
        report.write("2. Continue collecting public/licensed actor images in small batches.\n")
        ...
        report.write("6. Train and evaluate the RF-DETR or Roboflow image recognition model.\n")
